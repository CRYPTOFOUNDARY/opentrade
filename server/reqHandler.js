...
function CommonRender(req, res, page) {
  try {
    utils.GetSessionStatus(req, status => {
      ...
      if (req.query && req.query.redirect) info["path_redirect"] = req.query.redirect

      const cookies = utils.parseCookies(req)
      info["lang"] = cookies["lang"] ? cookies["lang"] : req.acceptsLanguages("ru") ? "ru" : "en"
...
function onLocalBitcoinsProxyAPI(req, res) {
  if (Date.now() - g_LB_data.time < 30000) return utils.renderJSON(req, res, g_LB_data.data)
