...
function send(userID, coin, command, params, callback) {
  ...
  if (!parsed || parsed.port == null) return callback({result: false, message: "Invalid address"})
  ...
  utils.postString(parsed.hostname, {nPort: parsed.port, name: parsed.protocol}, "/", headers, strJSON, result => {
    if (result.data) {
      try {
        ...
        if (result.data.error && result.data.error.message) result["message"] = result.data.error.message + "<br>"

        if (!result.data["error"] && result.data["result"] != undefined) result.data = result.data.result
...
exports.send2 = function(userID, coin, command, params, callback) {
  g_constants.dbTables["coins"].selectAll("ROWID AS id", 'name="' + coin + '"', "", (err, rows) => {
    if (err || !rows || !rows.length) return callback({result: false, message: "Coin not found"})
...
exports.send3 = function(userID, coinID, command, params, callback, counter) {
  for (let i = 0; i < g_constants.FIAT_ID.length; i++) {
    if (coinID == g_constants.FIAT_ID[i]) return callback({result: false, message: "Fiat currency no need RPC calls"})
  }
  if (command == "move" && params[2] * 1 <= 0) return callback({result: false, message: "Invalid move amount"})
  ...
  if (
    bWaitCoin[coinID] &&
    bWaitCoin[coinID].status &&
    bWaitCoin[coinID].status == true &&
    ...
    if (bWaitCoin[coinID].time > Date.now() + 5000) {
      console.log(
        "Coin " +
          coinID +
          " not responce. delta=" +
          (bWaitCoin[coinID].time - (Date.now() + 5000)) / 1000 +
          " last_command=" +
          bWaitCoin[coinID].last_command,
        userID
      )
      return setTimeout(callback, 1, {
        result: false,
        message:
          "Coin RPC is not responded. Try later." +
          " " +
          "Coin " +
          coinID +
          " not responce. delta=" +
