...
exports.onNewMessage = function(ws, req, messageObject) {
  if (messageObject.text.length == 0) return
  ...
  utils.GetSessionStatus(req, status => {
    ...
    IsUserBanned(status.id, ret => {
      ...
      if (messageObject.text.length > 150) messageObject.text = messageObject.text.substr(0, 150)
...
function IsUserBanned(userID, callback) {
  g_constants.dbTables["chatban"].selectAll("*", "userID=" + userID, "", (err, rows) => {
    if (err || !rows || !rows.length) return callback(false)
    ...
    g_constants.dbTables["chatban"].delete('userID="' + escape(userID) + '"', err => {
      if (err) {
        if (ws.readyState === WebSocket.OPEN)
          ws.send(JSON.stringify({request: "answer", message: "Error at unbanning user"}))
      } else {
...
exports.onBanUser = function(ws, req, messageObject) {
  ...
  utils.GetSessionStatus(req, status => {
    ...
    g_constants.dbTables["users"].selectAll(
      "ROWID AS id",
      'login="' + escape(messageObject.user) + '"',
      "",
      (err, rows) => {
        if (err || !rows || !rows.length) return

        g_constants.dbTables["chatban"].insert(
          rows[0].id,
          Date.now(),
          messageObject.info.endTime,
          JSON.stringify(messageObject.info.comment),
          e => {}
        )
      }
    )
...
exports.onDeleteMessage = function(ws, req, messageObject) {
  if (!messageObject["message"] || !messageObject["user"]) return
    ...
    GetLastMessages(messages => {
      ...
      for (let key in messages) {
        ...
        for (var i = 0; i < msg.length; i++) {
          if (
            msg[i].message.text == messageObject.message.text &&
            msg[i].user == messageObject.user &&
            messageObject.message.lang == key
          )
            continue
...
exports.onRequestMessages = function(ws) {
  ...
  GetLastMessages(messages => {
    let msgArray = []
    for (var key in messages) msgArray = msgArray.concat(messages[key])
...
function GetLastMessages(callback) {
  if (!g_constants.dbTables["KeyValue"]) return callback(chat)

  for (var key in chat) if (chat[key].length) return callback(chat)
...
function SaveMessage(msg) {
  ...
  var tmp =
    chat[msg.message.lang].length > 300
      ? chat[msg.message.lang].slice(chat[msg.message.lang].length - 300)
      : chat[msg.message.lang]
