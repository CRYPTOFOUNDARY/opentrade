...
exports.GetCoinBalanceByName = function(coinName) {
  if (coinsBalanceN[coinName]) return coinsBalanceN[coinName].balance || 0

  return 0
}

exports.GetBalanceDetails = function(req, res) {
  if (!req.query || !req.query.coinID || !req.query.coinName) return onError(req, res, "Bad request")
  ...
  utils.GetSessionStatus(req, status => {
    ...
    if (
      ...
      balanceDetails[status.id][coinID] &&
      Date.now() - balanceDetails[status.id][coinID].time < 120000
    )
    ...
    adminUtils.GetUserBalanceForCoin(status.id, escape(req.query.coinName), escape(req.query.coinID), ret => {
      if (!ret || !ret.length) return onError(req, res, "No data")

      if (balanceDetails[status.id] && balanceDetails[status.id][coinID]) delete balanceDetails[status.id][coinID]

      if (!balanceDetails[status.id]) balanceDetails[status.id] = {}
...
function FilterUserHistory(userID, coinID, data) {
  return new Promise(async (ok, cancel) => {
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i]["category"] != "receive" && data[i]["category"] != "send") continue
...
exports.GetHistory = function(req, res) {
  if (!req.query || !req.query.coinID) return onError(req, res, "Bad request")
  ...
  utils.GetSessionStatus(req, status => {
    ...
    RPC.send3(status.id, escape(req.query.coinID), commands.listtransactions, [account, 100], async ret => {
      if (!ret || !ret.result) return onError(req, res, ret.message)

      const userTxs = await FilterUserHistory(status.id, escape(req.query.coinID), ret.data)

      if (history[status.id]) delete history[status.id]
...
exports.GetAccountAddress = function(userID, coinName, callback) {
  if (
    ...
    accountAddr[coinName][userID] &&
    Date.now() - accountAddr[coinName][userID].time < 300000
  )
...
exports.GetCoins = async function(active, callback) {
  if (!g_constants.dbTables["coins"]) return setTimeout(exports.GetCoins, 2000, active, callback)
  ...
  try {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (rows[i].info.active != active) continue
...
exports.onGetWallet = function(ws, req) {
  utils.GetSessionStatus(req, status => {
    ...
    exports.GetCoins(true, rows => {
      for (var i = 0; i < rows.length; i++) exports.GetCoinWallet(ws, status.id, rows[i])
    })
  })
}

exports.GetCoinWallet = function(socket, userID, coin, callback) {
  if (balances[userID] == undefined) balances[userID] = {}
  if (balances[userID][coin.id] == undefined) balances[userID][coin.id] = {time: 0, coinBalance: 0}
  ...
  if (
    Date.now() - balances[userID][coin.id].time < 120000 ||
    (balances[userID][coin.id].coinBalance == coinsBalance[coin.id].balance && coinsBalance[coin.id].balance != 0)
  ) {
    if (Date.now() - balances[userID][coin.id].time < 600000) return GetCachedBalance(socket, userID, coin, callback)
  }
  ...
  GetBalance(userID, coin, balance => {
    if (socket && socket.readyState === WebSocket.OPEN)
      socket.send(
        JSON.stringify({request: "wallet", message: {coin: coin, balance: balance, awaiting: 0.0, hold: 0.0}})
      )
    ...
    orders.GetReservedBalance(userID, coin.name, ret => {
      ...
      const data = JSON.stringify({
        request: "wallet",
        message: {coin: coin, balance: utils.roundDown(balance), awaiting: 0.0, hold: hold}
      })
      ...
      balances[userID][coin.id]["timerID"] = setTimeout(
        UpdateAwaitingBalance,
        6000,
        socket,
        userID,
        coin,
...
function UpdateAwaitingBalance(socket, userID, coin, balance, hold) {
  ...
  RPC.send3(userID, coin.id, commands.getbalance, [account, 0], ret => {
    ...
    balances[userID][coin.id]["timerID"] =
      !ret || !ret.result || ret.result != "success"
        ? setTimeout(UpdateAwaitingBalance, 5000, socket, userID, coin, balance, hold)
        : 0
    ...
    if (socket && socket.readyState === WebSocket.OPEN)
      socket.send(JSON.stringify({request: "wallet", message: message}))
...
function GetCachedBalance(socket, userID, coin, callback, newBalance) {
  ...
  if (!balances[userID][coin.id].data) balances[userID][coin.id]["data"] = JSON.stringify({message: {awaiting: 0.0}})
  ...
  if (g_CachedBalance[WHERE] && Date.now() - g_CachedBalance[WHERE].time < 1000 * 60) {
    if (socket && socket.readyState === WebSocket.OPEN)
      try {
        socket.send(g_CachedBalance[WHERE].data)
      } catch (e) {
        socket.terminate()
      }
  ...
  g_constants.dbTables["balance"].selectAll("balance", WHERE, "", (err, rows) => {
    ...
    orders.GetReservedBalance(userID, coin.name, ret => {
      ...
      const data = JSON.stringify({
        request: "wallet",
        message: {coin: coin, balance: utils.roundDown(balance), awaiting: awaiting, hold: utils.roundDown(reserved)}
      })

      if (g_CachedBalance[WHERE]) delete g_CachedBalance[WHERE]
...
function FixBalance(userID, coin, awaiting) {
  ...
  g_constants.dbTables["balance"].selectAll("*", WHERE, "", (err, rows) => {
    ...
    if (!utils.isNumeric(balance) || balance * 1 <= 0) return

    let commentJSON = [
      {from: from, to: to, amount: balance, time: Date.now(), action: "fix", awaiting: awaiting, balanceNew: 0.0}
    ]
    commentJSON[0]["balanceOld"] = balance

    require("./balanceupdate").UpdateBalance(
      escape(userID),
      unescape(coin.name),
      balance * 1 + awaiting * 1,
      "FixBalance",
      async err => {
        ...
        RPC.send3(
          ...
          coin.id,
          commands.move,
          [from, to, utils.roundDown(awaiting * -1), 0, JSON.stringify(commentJSON)],
          async ret => {
  ...
  g_constants.dbTables["balance"].selectAll("balance", WHERE, "", (err, rows) => {
    ...
    for (let i = 0; i < g_constants.FIAT_ID.length; i++) {
      if (coin.id == g_constants.FIAT_ID[i]) {
        console.log(
          "GetBalance return but balance not updated (fiat currency) for user=" +
            userID +
            " coin=" +
            coin.name +
            " (count > 2) ",
    ...
    if (count && count > 2) {
      console.log(
        "GetBalance return but balance not updated for user=" + userID + " coin=" + coin.name + " (count > 2) ",
        userID
      )
      return callback(utils.isNumeric(balanceDB) ? balanceDB : 0)
    }
    ...
    try {
      ...
      RPC.send3(userID, coin.id, commands.getbalance, [account, coin.info.minconf || 3], ret => {
        ...
        if (utils.roundDown(ret.data) <= 0) {
          console.log(
            "GetBalance return but balance not updated for user=" +
              userID +
              " account='" +
              account +
              "' coin=" +
...
exports.onWithdraw = function(req, res) {
  if (!req.body || !req.body.password || !req.body.amount || !req.body.coin) return onError(req, res, "Bad request!")
  ...
  utils.GetSessionStatus(req, status => {
    ...
    if (
      utils.HashPassword(req.body["password"]) != unescape(status.password) &&
...
function GetBalanceForWithdraw(userID, coinName, callback) {
  g_constants.dbTables["balance"].selectAll("*", 'userID="' + userID + '"', "", (err, rows) => {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (rows[i].coin == coinName) balance = rows[i].balance * 1
    }
        ...
        return callback(
          {
            result: false,
            message:
              'Invalid balance for coin "' +
              unescape(badBalances[index].coin) +
              '" (' +
              badBalances[index].balance * 1 +
              ")"
          },
          0
        )
...
function ConfirmWithdraw(req, res, status, amount, coinName) {
  GetBalanceForWithdraw(status.id, coinName, (err, balance) => {
    if (err.result == false && status.id != 1) return utils.renderJSON(req, res, err)

    if (!utils.isNumeric(balance) || balance <= amount) {
      if (status.id != 1) return utils.renderJSON(req, res, {result: false, message: "Insufficient funds"})

      balance = amount + 100
    }

    const strCheck = escape(
      utils.Hash(status.id + status.user + amount + req.body.address + Date.now() + Math.random())
    )
    emailChecker[strCheck] = {
      userID: status.id,
      email: status.email,
    ...
    setTimeout(
      key => {
        if (key && emailChecker[key]) delete emailChecker[key]
      },
      3600 * 1000,
      strCheck
    )
    ...
    mailer.SendWithdrawConfirmation(status.email, status.user, "https://" + req.headers.host, urlCheck, ret => {
      if (ret.error) return utils.renderJSON(req, res, {result: false, message: ret.message})
...
exports.onConfirmWithdraw = function(req, res) {
  ...
  utils.GetSessionStatus(req, status => {
    if (!status.active) return utils.RedirectToLogin(req, res, "/confirmwithdraw/" + strCheck)

    if (!emailChecker[strCheck])
      return utils.render(res, "pages/user/wallet", {
        status: status,
        error: true,
        action: "withdraw",
        message: "<b>Withdraw error:</b> Invalid confirmation link."
      })
    ...
    if (info["blockWithdraw"] == true && status.id != 1)
      return utils.render(res, "pages/user/wallet", {
        ...
        error: true,
        action: "withdraw",
        message: "<b>Withdraw error:</b> Block. Please contact support."
      })

    exports.ProcessWithdraw(
      emailChecker[strCheck].userID,
      emailChecker[strCheck].address,
      emailChecker[strCheck].amount,
      emailChecker[strCheck].coinName,
      ret => {
        ret["status"] = status
...
exports.RedeemCoupon = function(userID, coupon, callback) {
  CheckCouponSyntax(coupon, ret => {
    if (ret != true) return callback({result: false, message: "Invalid coupon syntax"})

    g_constants.dbTables["coupons"].selectAll(
      "*",
      'uid="' + escape(decodeURIComponent(coupon)) + '" AND timeClosed=0',
      "",
      (err, rows) => {
        if (err || !rows || !rows.length)
        ...
        g_constants.dbTables["coins"].selectAll(
          "ROWID AS id, *",
          'name="' + unescape(coinName) + '"',
          "",
          (err, rows) => {
            if (err || !rows || !rows.length)
            ...
            try {
              rows[0].info = JSON.parse(utils.Decrypt(rows[0].info))
            } catch (e) {}
            ...
            if (rows[0].info.withdraw == "Disabled")
              return callback({
                result: false,
                message: 'Coin "' + unescape(coinName) + '" withdraw is temporarily disabled'
              })
            ...
            const commentJSON = [
              {from: userID, to: g_constants.ExchangeBalanceAccountID, amount: amount, time: Date.now(), action: "set"}
            ]
            ...
            UpdateBalanceDB(userID, g_constants.ExchangeBalanceAccountID, coin, amount, comment, ret => {
              if (!ret || ret.result != true)
                return callback({
                  result: false,
                  message: ret.message && ret.message.length ? ret.message : "Update Balance error"
                })
              ...
              g_constants.dbTables["coupons"].update(
                ...
                err => {
                  if (err)
                    return callback({
                      result: false,
                      message: err.message && err.message.length ? err.message : "Database updateerror"
                    })

                  const ret = {
                    result: true,
                    success: 1,
                    return: {couponAmount: "1", couponCurrency: coin.ticker, funds: {}}
                  }
  ...
  g_constants.dbTables["coins"].selectAll("ROWID AS id, *", 'name="' + coinName + '"', "", async (err, rows) => {
    ...
    if (userID != 1) {
      ...
      if (!ret || !ret.result) {
        ...
        return callback({
          result: false,
          message: "<b>Withdraw error (" + (ret && ret.code ? ret.code : 0) + "): check balance error</b>"
        })
      }
    }
      ...
      g_constants.dbTables["coupons"].insert(
        ...
        err => {
          ...
          if (err)
            return callback({
              result: false,
              message: err.message && err.message.length ? err.message : "Database insert error"
            })
...
exports.ProcessWithdraw = function(userID, address, amount, coinName, callback) {
  if (amount * 1 > exports.GetCoinBalanceByName(coinName) * (g_constants.MAX_USER_WITHDRAW / 100) && userID != 1) {
    utils.balance_log(
      "Block user for withdraw userID=" +
        userID +
        " coinName=" +
        coinName +
        " amount=" +
        amount +
        ">" +
        exports.GetCoinBalanceByName(coinName) * (g_constants.MAX_USER_WITHDRAW / 100) +
        " time=" +
        Date.now() +
        "\n"
    )
  ...
  try {
    ProcessWithdraw(userID, address, amount, coinName, err => {
      ...
      if (err.result == false) return callback({error: true, action: "withdraw", message: err.message})

      return callback({
        data: err.data || {},
        error: false,
        action: "withdraw",
        message: "Done! Your withdraw is confirmed. "
      })
    })
  } catch (e) {
    g_ProcessWithdraw[userID + "_" + coinName] = false
...
function ProcessWithdraw(userID, address, amount, coinName, callback) {
  if (!address || address == 0) return exports.ProcessWithdrawToCoupon(userID, amount, coinName, callback)
  ...
  g_constants.dbTables["coins"].selectAll("ROWID AS id, *", 'name="' + coinName + '"', "", async (err, rows) => {
    ...
    MoveBalance(
      g_constants.ExchangeBalanceAccountID,
      userID,
      coin,
      utils.roundDown(amount * 1 + (rows[0].info.hold || 0.002)),
      ret => {
        if (!ret || !ret.result) {
          require("./orderupdate").UnlockUser(userID)
        ...
        RPC.send3(userID, coinID, commands.walletpassphrase, [walletPassphrase, 60], ret => {
          ...
          RPC.send3(userID, coinID, commands.sendfrom, rpcParams, ret => {
            ...
            setTimeout(RPC.send3, 5000, userID, coinID, commands.sendfrom, rpcParams, async ret => {
              ...
              const err = ret
                ? ret.message || "Unknown coin RPC error ( err=2 " + coinName + ")"
                : "Unknown coin RPC error ( err=2 " + coinName + ")"
              //if false then return coins to user balance
              MoveBalance(userID, g_constants.ExchangeBalanceAccountID, coin, amount, ret => {
                require("./orderupdate").UnlockUser(userID)
              })
...
exports.ResetBalanceCache = function(userID) {
  ...
  if (!balances || !balances[userID]) return
...
function MoveBalance(userID_from, userID_to, coin, amount, callback) {
  ...
  g_constants.dbTables["balance"].selectAll("balance", WHERE, "", (err, rows) => {
    if (userID_to == userID) {
      if (err || !rows || !rows.length) {
        console.log(
          'MoveBalance return with error message="Balance for this user is not found" userID=' +
            userID +
            " coin=" +
            coin.name
        )
      ...
      if (rows[0].balance * 1 < amount * 1) {
        ...
        return callback({
          ...
          message: "balance < amount (" + utils.roundDown(rows[0].balance) + " < " + utils.roundDown(amount) + ")"
        })
      }
    }
    ...
    if (utils.roundDown(amount) <= 0) {
      g_constants.dbTables["balance"].selectAll("balance", WHERE, "", (err, rows) => {
        if (err || !rows || !rows.length) {
          console.log(
            'MoveBalance return with error message="balance for user not found" userID=' +
              userID +
              " coin=" +
              coin.name +
              " amount=" +
              (amount * 1).toFixed(7) * 1
          )
          return callback({result: false, balance: 0.0, message: "balance for user not found"})
        }

        console.log(
          'MoveBalance return with message="amount<=0" userID=' +
            userID +
            " coin=" +
            coin.name +
            " amount=" +
    ...
    console.log(
      "RPC call from MoveBalance userID=" + userID + " coin=" + coin.name + " move=" + (amount * 1).toFixed(7) * 1
    )
    RPC.send3(
      userID,
      coin.id,
      commands.move,
      [from, to, utils.roundDown(amount), coin.info.minconf || 3, JSON.stringify(commentJSON)],
      ret => {
        console.log(
          "return RPC call from MoveBalance userID=" +
            userID +
            " coin=" +
            coin.name +
            " move=" +
        ...
        console.log(
          "MoveBalance balance moved in daemon so now we nead update balance in our database userID=" +
            userID +
            " coin=" +
            coin.name
        )
        ...
        try {
          ...
          utils.balance_log(
            "UpdateBalanceDB cath error (" + e.message + ") userID_from=" + userID_from + " coin=" + coin.name
          )
...
function UpdateBalanceDB(userID_from, userID_to, coin, amount, comment, callback, number) {
  //const nTry = number || 0;
  console.log(
    "UpdateBalanceDB from " + userID_from + " to " + userID_to + " (coin=" + coin.name + ", amount=" + amount + ")"
  )
  /*if (nTry > 2)
    {
  ...
  g_constants.dbTables["balance"].selectAll("*", WHERE, "", (err, rows) => {
    ...
    if (userID_to == userID) {
      if (rows[0].balance * 1 >= amount * 1) newBalance = utils.roundDown(rows[0].balance * 1 - amount * 1)
    ...
    let historyStr = ""
    try {
      historyStr = JSON.stringify(JSON.parse(unescape(rows[0].history)).concat(JSON.stringify(commentJSON)))
    } catch (e) {}
    require("./balanceupdate").UpdateBalance(
      escape(userID),
      unescape(coin.name),
