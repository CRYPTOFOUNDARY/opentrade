...
exports.UpdateBalance = async function(userID, coin, newAmount, log = "", callback, c) {
  const counter = c || 0
  if (counter > 10) return callback({result: false, message: "UpdateBalance busy"})
  ...
  try {
    const rows = await g_constants.dbTables["balance"].Select(
      "*",
      "userID='" + userID + "' AND coin='" + escape(coin) + "'"
    )

    if (rows.length == 0)
      await g_constants.dbTables["balance"].Insert(
        userID,
        unescape(coin),
        utils.roundDown(newAmount),
        JSON.stringify({}),
      ...
      await g_constants.dbTables["balance"].Update(
        "balance='" + utils.roundDown(newAmount) + "'",
        "userID='" + userID + "' AND coin='" + escape(coin) + "'"
      )
