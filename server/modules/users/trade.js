      ...
      exports.GetAllOrders(data, ret => {
        let retMessage = {
          ...
          message: {
            result: true,
            data: {
              online: utils.GetValidSessionsCount(),
              allusers: utils.GetAllUsersCount(),
              orders: "",
              userOrders: "",
              historyUser: "",
              history: ""
...
exports.GetLastCoinHistory = function(coin) {
  if (!tradeHistory[coin.name] || !tradeHistory[coin.name].data)
    return GetTradeHistory([g_constants.share.TRADE_MAIN_COIN, coin.name], () => {})

  if (!tradeHistory[coin.name].data.length)
    tradeHistory[coin.name].data.push({
      volume: 0.0,
      price: 0.0,
      fromBuyerToSeller: 0.0,
      prev_frombuyertoseller: 0.0,
      buysell: "buy",
      prev_price: 0.0,
      prev_buysell: "buy"
    })
...
function GetCoins(coin1, coin2, callback) {
  ...
  if (g_CoinsInfo[WHERE]) g_CoinsInfo[WHERE].time = Date.now()

  console.log("GetCoins " + coin1 + " - " + coin2)
  try {
    g_constants.dbTables["coins"].selectAll("ROWID AS id, name, ticker, icon, info", WHERE, "", (err, rows) => {
      if (err || !rows || !rows.length || rows.length != 2) return callback({result: false, message: "Pair not found"})
...
function GetBalance(socket, status, data) {
  ...
  GetCoins(data[0], data[1], err => {
    if (!err || !err.result || !err.data || err.data.length != 2) {
      if (socket.readyState === WebSocket.OPEN)
        socket.send(
          JSON.stringify({request: "wallet", message: {result: false, message: err.message || "Unknown message"}})
        )
      return
    }

    for (var i = 0; i < err.data.length; i++) setTimeout(wallet.GetCoinWallet, 1, socket, status.id, err.data[i])
  })
}

exports.GetAllOrders = function(data, callback) {
  // callback({result: true, data: {}}); return;
  if (!data || !data.length || data.length != 2) return callback({result: false, message: "Bad Request"})
...
function GetUserTradeHistory(status, data, callback) {
  if (!status.active) return callback({result: true, data: []})
  ...
  g_constants.dbTables["history"].selectAll(
    "buyUserID, sellUserID, fromSellerToBuyer AS volume, fromBuyerToSeller, price, buysell, time",
    WHERE,
...
function GetTradeHistory(data, callback) {
  if (
    tradeHistory &&
    tradeHistory[data[1]] &&
    tradeHistory[data[1]].time &&
    Date.now() - tradeHistory[data[1]].time < 5000
  )
    return callback({result: true, data: tradeHistory[data[1]].data})

  g_constants.dbTables["history"].selectAll(
    "fromSellerToBuyer AS volume, fromBuyerToSeller, price, buysell, time",
    'coin="' + escape(data[1]) + '" AND coin_pair="' + escape(data[0]) + '"',
    "ORDER BY time*1 DESC LIMIT 200",
    (err, rows) => {
      ...
      if (!tradeHistory[data[1]]) tradeHistory[data[1]] = {}
      ...
      if (tradeHistory[data[1]].data.length > 1) {
        ...
        tradeHistory[data[1]].data[0]["prev_frombuyertoseller"] =
          tradeHistory[data[1]].data[1].fromBuyerToSeller / tradeHistory[data[1]].data[1].volume
...
function GetChartData(data, callback) {
  ...
  if (!chartData[data[1]]) chartData[data[1]] = {} //
  if (!chartData[data[1]][data[2]]) chartData[data[1]][data[2]] = {time: 0, data: []}

  if (Date.now() - chartData[data[1]][data[2]].time < 3600000)
    return callback({result: true, data: chartData[data[1]][data[2]].data})
  ...
  try {
    ...
    g_constants.dbTables["history"].selectAll(
      "SUM(fromSellerToBuyer*1) AS volume, AVG((fromBuyerToSeller/fromSellerToBuyer)*1000000) AS avg_10min, (time/" +
        group +
        ") AS t10min",
      'coin="' + escape(data[1]) + '" AND coin_pair="' + escape(data[0]) + '"',
      "GROUP BY t10min ORDER BY t10min*1 DESC LIMIT 200",
      (err, rows) => {
        if (err || !rows) callback({result: false, data: []})
