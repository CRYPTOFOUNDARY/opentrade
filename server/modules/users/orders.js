...
function onError(req, res, message) {
  if (!res && req.callback) return req.callback({result: false, message: message})

  utils.renderJSON(req, res, {result: false, message: message})
}
function onSuccess(req, res, data) {
  if (!res && req.callback) return req.callback({result: true, data: data})
...
exports.CloseAllUserOrders = function(userID, coinName) {
  const WHERE =
    coinName != g_constants.share.TRADE_MAIN_COIN
      ? 'userID="' +
        userID +
        '" AND amount*price>0 AND amount*1>0 AND coin="' +
        escape(coinName) +
        '"  AND buysell="sell"'
      : 'userID="' + userID + '" AND amount*1>0 AND buysell="buy"'
...
exports.CloseUserOrder = async function(userID, orderROWID, callback) {
  ...
  if (IsOrderLocked(orderROWID)) return callback(false, "Order is locked")
...
exports.CloseOrder = function(req, res) {
  if (!req || !req.body || !req.body.orderID) return onError(req, res, req.message || "Bad request")
  ...
  utils.GetSessionStatus(req, status => {
    ...
    exports.CloseUserOrder(status.id, req.body.orderID, (result, data) => {
      if (result == false) return onError(req, res, data)
...
exports.SubmitOrder = function(req, res) {
  utils.GetSessionStatus(req, status => {
    ...
    utils.CheckCoin(unescape(req.body.coin), err => {
      ...
      const queryCoin = req.body.order == "buy" ? escape(g_constants.share.TRADE_MAIN_COIN) : escape(req.body.coin)
      ...
      g_constants.dbTables["balance"].selectAll("*", WHERE, "", (err, rows) => {
        if (err || !rows || !rows.length)
          return onError(req, res, err && err.message ? err.message : "User balance not found")

        const fullAmount =
          req.body.order == "buy"
            ? utils.roundDown(
                req.body.amount * req.body.price + g_constants.share.TRADE_COMISSION * req.body.amount * req.body.price
              )
            : utils.roundDown(req.body.amount * 1)

        if (fullAmount * 1 < 0.00001)
          return onError(req, res, "Bad order total ( total < 0.00001 ) " + "( " + fullAmount * 1 + " < 0.00001 )")
        ...
        if (!IsValidBalance(rows[0].balance * 1 - fullAmount))
          return onError(req, res, "Insufficient funds ( " + rows[0].balance * 1 + " < " + fullAmount + " )")
...
function IsValidBalance(balance) {
  if (!utils.isNumeric(balance)) return false

  if (balance * 1.0 < 0.0) return false
...
exports.GetReservedBalance = function(userID, coinName, callback) {
  if (coinName != g_constants.share.TRADE_MAIN_COIN) {
    g_constants.dbTables["orders"].selectAll(
      "SUM(amount*1) AS result",
      'userID="' + userID + '" AND amount*price>0 AND coin="' + coinName + '" ' + 'AND buysell="sell"',
      "",
      (err, rows) => {
  ...
  g_constants.dbTables["orders"].selectAll(
    "SUM(amount*price) AS result",
    'userID="' + userID + '" AND amount*price>0 AND buysell="buy"',
    "",
    (err, rows) => {
      if (err || !rows) return callback({result: "fail", message: err.message || "Database error"})
...
exports.GetUserOrders = function(userID, coins, callback) {
  ...
  if (coins.length) WHERE += '"  AND amount*price>0 AND amount*1>0 AND ( '

  for (let i = 0; i < coins.length; i++) {
    WHERE += " coin='" + coins[i].name + "' "
    if (i != coins.length - 1) WHERE += " OR "
    else WHERE += " ) "
  }

  g_constants.dbTables["orders"].selectAll("ROWID AS id, *", WHERE, "ORDER BY time*1 DESC LIMIT 200", (err, rows) => {
    if (err) return callback({result: false, message: err.message || "Unknown database error"})
...
exports.GetAllOrders = function(coinsOrigin, callback) {
  ...
  if (unescape(coins[0].name) == g_constants.share.TRADE_MAIN_COIN) coins = [coinsOrigin[1], coinsOrigin[0]]

  const coin0 = unescape(coins[0].name)

  if (coins.length != 2) return callback({result: false, message: "Coins error"})
  ...
  g_constants.dbTables["orders"].selectAll(
    "SUM(amount*1) AS amount, coin, price, time",
    'coin="' + escape(coin0) + '" AND buysell="buy" AND amount*price>0 AND amount*1>0 ',
    "GROUP BY price*1000000 ORDER BY price*1000000 DESC LIMIT 30",
    (err, rows) => {
      g_constants.dbTables["orders"].selectAll(
        "SUM(amount*1) AS amount, coin, price, time",
        'coin="' + escape(coin0) + '" AND buysell="sell" AND amount*price>0 AND amount*1>0 ',
        "GROUP BY price*1000000 ORDER BY price*1000000 LIMIT 30",
        (err2, rows2) => {
          g_constants.dbTables["orders"].selectAll(
            "SUM(amount*1) AS sum_amount, SUM(amount*price) AS sum_amount_price",
            'amount*price>0 AND amount*1>0 AND coin="' + escape(coin0) + '"',
            "GROUP BY buysell",
            (err3, rows3) => {
...
async function AddOrder(status, WHERE, newBalance, req, res, c) {
  const counter = c || 0
  if (counter > 5) return onError(req, res, "Coin locked. Please try later")
  ...
  if (
    !utils.isNumeric(amount) ||
    !utils.isNumeric(price) ||
    !utils.isNumeric(balance) ||
    amount <= 0 ||
    price <= 0 ||
    balance < 0
  ) {
  ...
  require("./balanceupdate").UpdateBalance(status.id, unescape(req["queryCoin"]), balance, "AddOrder", err => {
    ...
    g_constants.dbTables["orders"].insert(
      ...
      async err => {
        ...
        if (userOrders[status.id]) delete userOrders[status.id]
...
function LockOrder(id) {
  for (let i = 0; i < g_LockedOrders.length; i++) if (g_LockedOrders[i] == id) return
...
function IsOrderLocked(id) {
  for (let i = 0; i < g_LockedOrders.length; i++) if (g_LockedOrders[i] == id) return true
...
exports.ProcessExchange = function(coin) {
  if (g_LockedCoins[coin]) return

  g_LockedCoins[coin] = true
  const SQL =
    "SELECT * FROM (" +
    'SELECT * FROM (SELECT ROWID as id, * FROM orders where coin="' +
    coin +
    '"  AND amount*price > 0 AND amount*100>0.000001 AND price*100>0.000001 AND buysell="buy" ORDER BY price*1 DESC, time*1 LIMIT 1 ) ' +
    "UNION " +
    'SELECT * FROM (SELECT ROWID as id, * FROM orders where coin="' +
    coin +
    '"  AND amount*price > 0  AND amount*100>0.000001 AND price*100>0.000001 AND buysell="sell"  ORDER BY price*1, time*1 LIMIT 1 )' +
    ") ORDER BY buysell"
  ...
  function RunExchange(buyOrder, sellOrder, callback) {
    ...
    const newBuyAmount =
      buyOrder.amount * 1 < sellOrder.amount * 1 ? 0 : utils.roundDown(buyOrder.amount * 1 - sellOrder.amount * 1)
    const newSellAmount =
      buyOrder.amount * 1 < sellOrder.amount * 1 ? utils.roundDown(sellOrder.amount * 1 - buyOrder.amount * 1) : 0
    ...
    const buyerChange =
      priority == "buyer" ? utils.roundDown((buyOrder.price * 1 - sellOrder.price * 1) * fromSellerToBuyer) : 0.0
    ...
    UpdateOrders(newBuyAmount, newSellAmount, buyOrder.id, sellOrder.id, err => {
      ...
      UpdateBalances(buyOrder, sellOrder, fromSellerToBuyer, fromBuyerToSeller, buyerChange, comission, err => {
        UpdateHistory(buyOrder, sellOrder, fromSellerToBuyer, fromBuyerToSeller, buyerChange, comission, err => {
          ...
          if (userOrders[sellOrder.userID]) delete userOrders[sellOrder.userID]
          if (userOrders[buyOrder.userID]) delete userOrders[buyOrder.userID]
    ...
    g_constants.dbTables["referals"].selectAll("*", WHERE, "", (err, rows) => {
      ...
      for (var i = 0; i < g_constants.DONATORS.length; i++) {
        if (g_constants.DONATORS[i].percent && g_constants.DONATORS[i].userID)
          exports.AddBalance(
            g_constants.DONATORS[i].userID,
            (donatorsFee * (g_constants.DONATORS[i].percent * 1 - 1)) / 100.0,
            price_pair,
            () => {},
            buerID,
            "From OpenTrade comission"
          )
      }
    })
  }
  ...
  async function UpdateOrders(newBuyAmount, newSellAmount, buyOrderID, sellOrderID, callback) {
    try {
      const rows = await g_constants.dbTables["orders"].Select(
        "ROWID AS id, coin, amount, info",
        "ROWID=" + buyOrderID + " OR ROWID=" + sellOrderID
      )
      ...
      const logSell0 =
        rows[0].id == buyOrderID ? JSON.parse(unescape(rows[1].info)) : JSON.parse(unescape(rows[0].info))
      ...
      await g_constants.dbTables["orders"].Update(
        'amount="' +
          newBuyAmount +
          '", info="' +
          escape(
            JSON.stringify(
              logBuy.concat([{action: "updtB", amount: newBuyAmount, old0: rows[0].amount, old1: rows[1].amount}])
            )
          ) +
          '"',
        'ROWID="' + buyOrderID + '"'
      ...
      await g_constants.dbTables["orders"].Update(
        'amount="' +
          ...
          escape(
            JSON.stringify(
              logSell.concat([{action: "updtS", amount: newSellAmount, old0: rows[0].amount, old1: rows[1].amount}])
            )
          ) +
          '"',
        'ROWID="' + sellOrderID + '"'
      ...
      require("./orderupdate").DeleteOrder(
        rows[0].coin,
        "(amount*price <= 0 OR amount*1 <= 0 OR price*1 <= 0) AND time*1+3600*48*1000 < " + Date.now(),
        err => {}
      )
...
exports.AddBalance = async function(userID, count, coin, callback, userFrom, comment) {
  ...
  g_constants.dbTables["balance"].selectAll("*", WHERE, "", (err, rows) => {
    ...
    if (userFrom && comment) {
      g_constants.dbTables["payments"].insert(userID, userFrom, count, unescape(coin), Date.now(), comment, err => {})
    }

    if (rows.length)
      return require("./balanceupdate").UpdateBalance(userID, unescape(coin), balance, "AddBalance", callback)
