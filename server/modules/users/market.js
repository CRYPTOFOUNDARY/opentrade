...
function ProcessExchange() {
  wallet.GetCoins(true, ret => {
    for (var i = 0; i < ret.length; i++) setTimeout(orders.ProcessExchange, 1, ret[i].name)
...
function UpdateExchangeSummary() {
  utils.getJSON("https://trade.multicoins.org/api/v1/public/getmarkets", (status, data) => {
    if (status != 200 || !data || !data.result || data.success != true) return

    for (let i = 0; i < data.result.length; i++) {
      if (data.result[i].IsActive != true) continue

      const BTC = data.result[i].MarketCurrency
      const MC = data.result[i].BaseCurrency

      utils.getJSON(
        "https://trade.multicoins.org/api/v1/public/getmarketsummary?market=" + MC + "-" + BTC + "&period=24",
        (status2, data2) => {
          if (status2 != 200 || !data2 || data2.success != true || !data2.result) return
...
exports.UpdateMarket = function() {
  ...
  g_constants.dbTables["coins"].selectAll("ROWID AS id, name, ticker, icon, info", "", "ORDER BY id", (err, rows) => {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (rows[i].info.active != true) continue
