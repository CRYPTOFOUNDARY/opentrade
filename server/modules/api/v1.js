...
function SetCache(method, timeCached, data) {
  if (g_Cache[method]) delete g_Cache[method]
...
exports.onGetMarkets = function(req, res) {
  ...
  if (ret) return onSuccess(req, res, ret)

  g_constants.dbTables["coins"].selectAll("name, ticker, info", "", "", (err, rows) => {
    if (err || !rows) return onError(req, res, err && err.message ? err.message : "unknown database error")
...
exports.onGetOrderbook = function(req, res) {
  ...
  if (!dataParsed || !dataParsed.query) return onError(req, res, "Bad request")

  const queryStr = querystring.parse(dataParsed.query)
  if (!queryStr.market) return onError(req, res, 'Bad request. Parameter "market" not found')

  const data = queryStr.market.split("-")
  if (!data) return onError(req, res, 'Bad request. Parameter "market" is invalid')
  ...
  trade.GetAllOrders(data, ret => {
    ...
    if (type == "both" || type == "buy") {
      for (var i = 0; i < ret.data.buy.length; i++) {
        if (ret > 200) break
        if (!ret.data.buy[i].amount || !ret.data.buy[i].price) continue
    ...
    if (type == "both" || type == "sell") {
      for (var i = 0; i < ret.data.sell.length; i++) {
        if (ret > 200) break
        if (!ret.data.sell[i].amount || !ret.data.sell[i].price) continue
...
exports.onGetMarketSummary = function(req, res) {
  ...
  const period =
    queryStr.period &&
    (queryStr.period == 24 || queryStr.period == 250 || queryStr.period == 1000 || queryStr.period == 6000)
      ? queryStr.period * 1
      : 24

  console.log("period=" + period + " queryStr=" + JSON.stringify(queryStr))
  if (!utils.isNumeric(period)) return onError(req, res, "Bad request. Period is not numeric")

  const data = queryStr.market.split("-")
  if (!data || !data.length || data.length != 2) return onError(req, res, 'Bad request. Parameter "market" is invalid')
  ...
  g_constants.dbTables["coins"].selectAll("name, ticker, info, icon", 'ticker="' + data[1] + '"', "", (err, rows) => {
    ...
    if (!rows.length) return onError(req, res, "ticker " + data[1] + " not found")
    ...
    g_constants.dbTables["history"].selectAll(
      "max((fromBuyerToSeller*1)/fromSellerToBuyer) AS Height, min((fromBuyerToSeller*1)/fromSellerToBuyer) AS Low, sum(fromSellerToBuyer*1) AS Volume",
      WHERE,
      "GROUP BY coin",
      (err, rows) => {
        if (err || !rows) {
        ...
        g_constants.dbTables["orders"].selectAll(
          "count(price*1) as count_buy, max(price*1) AS Bid",
          "(coin='" + COIN.name + "' AND amount*price>0 AND buysell='buy')",
          "",
          (err, rows) => {
            if (rows.length) {
            ...
            g_constants.dbTables["orders"].selectAll(
              "count(price*1) as count_sell, min(price*1) AS Ask",
              "(coin='" + COIN.name + "' AND amount*price>0 AND buysell='sell')",
              "",
              (err, rows) => {
                if (rows.length) {
  ...
  g_constants.dbTables["coins"].selectAll("name, ticker, info", 'ticker="' + data[1] + '"', "", (err, rows) => {
    try {
      ...
      g_constants.dbTables["history"].selectAll(
        "ROWID AS id, *",
        WHERE,
        "ORDER BY time*1 DESC LIMIT 200",
        (err, rows) => {
          if (err || !rows) return onError(req, res, err && err.message ? err.message : "unknown database error")
...
function SubmitOrder(req, res, buysell) {
  try {
    ...
    const queryStr = querystring.parse(dataParsed.query)
    if (!queryStr.apikey || !queryStr.nonce || !queryStr.market || !queryStr.quantity || !queryStr.rate)
      throw new Error("Bad request. Required parameter (apikey or nonce or market or quantity or rate) not found.")
    ...
    utils.GetCoinFromTicker(currency[1], coin => {
      if (!coin || !coin.name) return onError(req, res, "Coin ticker not found")
...
exports.onMarketCancel = function(req, res) {
  try {
    ...
    const queryStr = querystring.parse(dataParsed.query)
    if (!queryStr.apikey || !queryStr.nonce || !queryStr.uuid)
      throw "Bad request. Required parameter (apikey or nonce or uuid) not found."
    ...
    CheckAPIkey(queryStr.apikey, req.headers["apisign"], fullUrl, ret => {
      try {
        ...
        g_constants.dbTables["orders"].selectAll("ROWID AS id", 'uuid="' + queryStr.uuid + '"', "", (err, rows) => {
          if (err || !rows || !rows.length) return onError(req, res, "Order with this uuid not found")
...
exports.onMarketGetOpenOrders = function(req, res) {
  try {
    ...
    const queryStr = querystring.parse(dataParsed.query)
    if (!queryStr.apikey || !queryStr.nonce || !queryStr.market)
      throw new Error("Bad request. Required parameter (apikey or nonce or market) not found.")
      ...
      CheckAPIkey(queryStr.apikey, req.headers["apisign"], fullUrl, ret => {
        try {
          ...
          orders.GetUserOrders(ret.key.userid, [coin], err => {
            if (err.result != true) return onError(req, res, err.message)
...
exports.onAccountGetBalance = function(req, res) {
  try {
    ...
    if (!queryStr.apikey || !queryStr.nonce || !queryStr.currency)
      throw new Error("Bad request. Required parameter (apikey or nonce or currency) not found.")
          ...
          wallet.GetCoinWallet(null, ret.key.userid, coin, data => {
            try {
              ...
              return onSuccess(req, res, {
                Currency: message.coin.ticker,
                Balance: utils.roundDown(balance + awaiting + hold),
                Available: balance.toFixed(8) * 1,
                Pending: awaiting.toFixed(8) * 1
              })
            } catch (e) {
              return onError(req, res, e.message)
            }
...
exports.onGenerateAPIkey = function(req, res) {
  utils.GetSessionStatus(req, status => {
    ...
    g_constants.dbTables["apikeys"].selectAll("*", 'userid="' + status.id + '"', "", (err, rows) => {
      ...
      g_constants.dbTables["apikeys"].insert(status.id, keyPub, 0, 0, 0, JSON.stringify({privKey: keyPriv}), err => {
...
exports.onEditAPIkey = function(req, res) {
  utils.GetSessionStatus(req, status => {
    ...
    const SET =
      'read="' +
      MakeFlag(req.body.read) +
      '", write="' +
      MakeFlag(req.body.write) +
      '", withdraw="' +
      MakeFlag(req.body.withdraw) +
      '"'
    g_constants.dbTables["apikeys"].update(
      SET,
      'userid="' + status.id + '" AND key="' + escape(req.body.key) + '"',
      err => {
...
exports.onAccountGetDepositAddress = function(req, res) {
  const dataParsed = url.parse(req.url)
  if (!dataParsed || !dataParsed.query || !req.headers["apisign"]) return onError(req, res, "Bad request")
    ...
    CheckAPIkey(queryStr.apikey, req.headers["apisign"], fullUrl, ret => {
      try {
        ...
        wallet.GetAccountAddress(ret.key.userid, coin.name, ret => {
          if (ret.result != "success" || !ret.data) return onError(req, res, ret.message)
...
exports.onAccountGetOrder = function(req, res) {
  try {
    ...
    const queryStr = querystring.parse(dataParsed.query)
    if (!queryStr.apikey || !queryStr.nonce || !queryStr.uuid)
      throw new Error("Bad request. Required parameter (apikey or nonce or uuid) not found.")
      ...
      try {
        ...
        g_constants.dbTables["orders"].selectAll("ROWID AS id, *", 'uuid="' + queryStr.uuid + '"', "", (err, rows) => {
          ...
          g_constants.dbTables["coins"].selectAll(
            "name, ticker",
            'name="' + rows[0].coin + '" OR name="' + rows[0].price_pair + '"',
            "",
            (err, r) => {
              if (err || !r || r.length != 2) return onError(req, res, "Coin name error")
...
exports.onRedeemCoupon = function(req, res) {
  ...
  if (!queryStr.coupon) return onError(req, res, "Bad request. Required parameter (coupon) not found.")
...
exports.onAccountWithdraw = function(req, res) {
  ...
  if (!utils.isNumeric(queryStr.quantity)) return onError(req, res, "Bad request. quantity is not numeric value")
  if (queryStr.quantity < 0.00001) return onError(req, res, "Bad request. quantity < 0.00001 (is too low)")
        ...
        try {
          ...
          if (queryStr.address) {
            wallet.ProcessWithdraw(ret.key.userid, queryStr.address, queryStr.quantity, coin.name, ret => {
              if (ret.error) return onError(req, res, ret.message)
          ...
          utils.GetSessionStatus(req, status => {
            if (!status.active || status.id != 1) return onError(req, res, "This operation is allowed for root only!")
...
function CheckAPIkey(strKey, strSign, strQuery, callback, req) {
  g_constants.dbTables["apikeys"].selectAll("*", 'key="' + escape(strKey) + '"', "", (err, rows) => {
    ...
    try {
      ...
      if (hash != strSign) return callback({success: false, message: "bad api signature"})
