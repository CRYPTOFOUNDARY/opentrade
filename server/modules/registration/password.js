...
exports.onPassworReset = async function(req, res) {
  try {
    if (
      req.body &&
      req.body["check"] &&
      req.body["checked-email"] &&
      emailChecker[req.body["check"]] &&
      emailChecker[req.body["check"]]["email"] &&
...
exports.onConfirmReset = function(req, res) {
  ...
  if (!emailChecker[strCheck]) {
    utils.render(res, "pages/registration/new_password", {
      error: true,
  ...
  utils.render(res, "pages/registration/new_password", {
    error: false,
    message: "Almost ready. Type new password",
    strCheck: strCheck,
    email: emailChecker[strCheck].email
  })
}
...
function validateForm(request, callback) {
  return new Promise((ok, cancel) => {
    if (!request.body || !request.body["email"]) return cancel(new Error("Bad Request"))
