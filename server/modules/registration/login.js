...
exports.onSubmit = async function(req, res) {
  try {
    ...
    if (
      utils.HashPassword(req.body["password"]) != unescape(ret.info.password) &&
...
exports.VerifyPin = function(req, res) {
  ...
  if (check.pin != req.body["pin"]) return LoginError(req, res, "ERROR: Not verified!")
...
function validateForm(request) {
  return new Promise((ok, cancel) => {
    if (!request.body || !request.body["username"] || !request.body["password"]) return cancel(new Error("Bad Request"))
