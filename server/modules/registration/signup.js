...
exports.onSubmit = async function(req, res) {
  ...
  function validateForm(req) {
    return new Promise((ok, cancel) => {
      ...
      if (!utils.ValidateEmail(req.body["email"])) return cancel(new Error("Ivalid email"))
  ...
  function SendConfirmEmail(req, res) {
    ...
    mailer.SendSignupConfirmation(req.body["email"], "https://" + req.headers.host, urlCheck, ret => {
      if (ret.error) return SignupError(req, res, ret.message)
...
function InsertNewUser(user, email, password, res, IP) {
  ...
  g_constants.dbTables["users"].insert(user, email, password, info, err => {
    if (err)
      return utils.render(res, "pages/registration/signup_confirm", {
        error: true,
        message: "Something wrong (( Please try again. (" + (err.message || JSON.stringify(err)) + ")"
      })
    ...
    g_constants.dbTables["users"].selectAll(
      "ROWID AS id, *",
      'login="' + escape(user) + '" AND email="' + escape(email) + '"',
      "",
      (err, rows) => {
