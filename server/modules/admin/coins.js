...
exports.onSupport = function(ws, req, data) {
  utils.GetSessionStatus(req, status => {
    ...
    admin_utils.GetUserRole(status.id, info => {
      if (!info.role || info.role != "Support") return
  ...
  function SupportCoin(coin, ws, action) {
    ...
    if (action == "start_withdraw") coin.info["withdraw"] = "Enabled"
    if (action == "start_orders") coin.info["orders"] = "Enabled"
    if (action == "stop_withdraw") coin.info["withdraw"] = "Disabled"
    if (action == "stop_orders") coin.info["orders"] = "Disabled"
    ...
    g_constants.dbTables["coins"].update(
      'info="' + escape(utils.Encrypt(JSON.stringify(coin.info) || "{}")) + '"',
      "ROWID=" + coin.id,
      err => {
        if (!err) apiV1.ResetCache("GetMarkets")

        if (ws.readyState === WebSocket.OPEN)
          ws.send(
            JSON.stringify({
              request: "coininfo_updated",
              message: {coin: coin.name, info: coin.info, trading: g_constants.share.tradeEnabled}
            })
          )
      }
    )
    ...
    err => {
      ...
      g_constants.dbTables["coins"].update(
        "ticker='" +
          ...
          "', " +
          "address='" +
          escape(utils.Encrypt(data.address)) +
          "', " +
          "rpc_user='" +
          escape(utils.Encrypt(data.rpc_user)) +
          "', " +
          "rpc_password='" +
          escape(utils.Encrypt(data.rpc_password)) +
          "', " +
          "info='" +
          escape(utils.Encrypt(data.info || "{}")) +
          "'",
