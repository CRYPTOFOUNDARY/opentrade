...
exports.GetUserRole = function(user, callback) {
  try {
    g_constants.dbTables["users"].selectAll("ROWID AS id, info", "id=" + escape(user), "", (err, rows) => {
      if (err || !rows || !rows.length) return callback({role: "User"})

      let oldInfo = JSON.parse(unescape(rows[0].info))
      if (!oldInfo.role) oldInfo["role"] = "User"
...
exports.BlockUserForWithdraw = function(userID) {
  g_constants.dbTables["users"].selectAll("ROWID AS id, info", "id=" + userID, "", (err, rows) => {
    ...
    if (!info["blockWithdraw"]) info["blockWithdraw"] = true
...
exports.GetCoinBalance = function(coinName, callback) {
  g_constants.dbTables["balance"].selectAll(
    "SUM(balance*1) AS sum_balance",
    'coin="' + coinName + '" AND balance*1>0',
    "",
    (err, rows) => {
      ...
      if (coinName == g_constants.share.TRADE_MAIN_COIN) {
        g_constants.dbTables["orders"].selectAll(
          "SUM(amount*price) AS blocked",
          'buysell="buy" AND amount*price>0',
          "",
          (err2, rows2) => {
        ...
        g_constants.dbTables["orders"].selectAll(
          "SUM(amount*1) AS blocked",
          'coin="' + coinName + '" AND buysell="sell" AND amount*price > 0',
          "",
          (err2, rows2) => {
            if (err2 || !rows2 || !rows2.length) rows2 = [{blocked: 0}]
            return callback({balance: rows[0].sum_balance, blocked: rows2[0].blocked})
...
exports.onGetCoinBalance = function(req, res) {
  ...
  utils.GetSessionStatus(req, status => {
    if (status.id != 1) return onError(req, res, "User is not root")
...
exports.onFindBannedChatUser = function(req, res) {
  ...
  utils.GetSessionStatus(req, status => {
    ...
    g_constants.dbTables["users"].selectAll(
      "ROWID AS id, login",
      'login GLOB "' + query + '"',
      "ORDER BY id LIMIT 10",
      (err, rows) => {
        if (err || !rows || !rows.length) return onError(req, res, "User is not root")

        let WHERE = "("
        for (var i = 0; i < rows.length; i++) {
          WHERE += ' userID="' + rows[i].id + '" '
          if (i + 1 < rows.length) WHERE += " OR "
        }
        WHERE += ") "
...
exports.onFindUserBalances = function(req, res) {
  if (!req.body) return onError(req, res, "Bad request")
...
exports.FixAllBalances = function() {
  if (!g_constants.dbTables["orders"]) return setTimeout(exports.FixAllBalances, 1000)

  g_constants.dbTables["orders"].selectAll(
    "*",
    "amount*price > 0 AND amount*1 > 0",
    "ORDER BY userID*1",
    (err, rows) => {
      if (err || !rows || !rows.length) return
...
exports.FixBalance = function(userID, coinName, callback, force) {
  ...
  console.log(chalk.red("FixBalance"), userID, coinName, callback, force)
    ...
    exports.GetUserBalanceForCoin(userID, coinName, coinID, ret => {
      ...
      if (!force) {
        if (Math.abs(realBalance - ret[0].balance * 1) < 0.0001) {
          if (ret[0].balance * 1 > 0 && realBalance * 1 > 0) return callback({result: true, timeout: 100})
        }
      ...
      const WHERE =
        coinName != g_constants.share.TRADE_MAIN_COIN
          ? 'userID="' + userID + '" AND coin="' + coinName + '"  AND buysell="sell"'
          : 'userID="' + userID + '" AND buysell="buy"'

      g_constants.dbTables["orders"].delete(WHERE, ret => {
        balance.UpdateBalance(
          userID,
          unescape(coinName),
          realBalance * 1 + (ret ? 0 : blocked * 1),
          "admin fix balance",
          err => {
            if (err) {
            ...
            if (force) exports.UnlockUserForWithdraw(userID)
...
exports.GetTransactionBalance = function(userID, coinID, txid) {
  return new Promise((ok, cancel) => {
    RPC.send3(userID, coinID, "gettransaction", [txid], ret => {
      if (!ret || !ret.result || !ret.data) return ok(0)
  ...
  return new Promise(async (ok, cancel) => {
    ...
    try {
      for (let i = 0; i < data.length; i++) {
        if (data[i]["confirmations"] && data[i]["confirmations"] < 3) continue

        if (data[i]["category"] == "receive") ret.deposit += (data[i]["amount"] || 0) * 1

        if (data[i]["category"] != "send") continue
...
exports.GetUserBalance = function(userID, coinsArray, index, result, callback, request, responce) {
  ...
  RPC.send3(userID, coinsArray[index].id, "listtransactions", [utils.Encrypt(userID), 10000], async ret => {
    ...
    const query1 =
      g_constants.share.TRADE_MAIN_COIN == coinsArray[index].name
    ...
    g_constants.dbTables["history"].selectAll("SUM(" + query1.from + ") AS buy", query1.WHERE, "", (err, rows) => {
      ...
      const query2 =
        g_constants.share.TRADE_MAIN_COIN == coinsArray[index].name
          ...
          : {
              WHERE: 'coin="' + coinsArray[index].name + '" AND sellUserID="' + userID + '"',
              from: "fromSellerToBuyer*1"
            }
        ...
        const query3 =
          g_constants.share.TRADE_MAIN_COIN == coinsArray[index].name
            ...
            : {
                WHERE:
                  'coin="' +
                  coinsArray[index].name +
                  '" AND userID="' +
                  userID +
                  '" AND amount*price > 0 AND buysell="sell"',
                mul: "1"
              }

        g_constants.dbTables["orders"].selectAll(
          "SUM(amount*" + query3.mul + ") AS blocked",
          query3.WHERE,
          "",
          (err, rows) => {
            ...
            g_constants.dbTables["balance"].selectAll(
              "balance",
              'coin="' + coinsArray[index].name + '" AND userID="' + userID + '" ',
              "",
              (err, rows) => {
                if (!err && rows && rows.length == 1) retJSON.balance = utils.roundDown(rows[0].balance)

                g_constants.dbTables["coupons"].selectAll(
                  "SUM(amount*1) AS couponDep",
                  'coin="' + coinsArray[index].name + '" AND userTo="' + userID + '"',
                  "",
                  (err, rows) => {
                    if (!err && rows && rows.length == 1 && utils.isNumeric(rows[0].couponDep))
                      retJSON.deposit += rows[0].couponDep * 1

                    g_constants.dbTables["coupons"].selectAll(
                      "SUM(amount*1) AS couponW",
                      'coin="' + coinsArray[index].name + '" AND userFrom="' + userID + '"',
                      "",
                      (err, rows) => {
                        if (!err && rows && rows.length == 1 && utils.isNumeric(rows[0].couponW))
                          retJSON.withdraw += rows[0].couponW * -1

                        g_constants.dbTables["payments"].selectAll(
                          "SUM(volume*1) AS fee",
                          'coin="' +
                            coinsArray[index].name +
                            '" AND volume*1<1 AND volume*1>0 AND userTo="' +
                            userID +
                            '"',
                          "",
                          (err, rows) => {
                            if (!err && rows && rows.length == 1 && utils.isNumeric(rows[0].fee))
                              retJSON.payouts += rows[0].fee * 1
...
exports.onFixBalance = function(req, res) {
  ...
  if (!req.query || !req.query.coin || !req.query.userID) return onError(req, res, "Bad request")
  ...
  utils.GetSessionStatus(req, status => {
    ...
    exports.FixBalance(
      ...
      req.query.coin,
      ret => {
        if (!ret || !ret.result) return onError(req, res, "Fix balance error")
        return onSuccess(req, res, ret)
      },
      true
    )
...
exports.onFindUser = function(req, res) {
  if (!req.body || !req.body.user) return onError(req, res, "Bad request")
  ...
  utils.GetSessionStatus(req, status => {
    ...
    g_constants.dbTables["users"].selectAll(
      "ROWID AS id, *",
      'login GLOB "' + query + '" OR email GLOB "' + query + '" OR ROWID="' + query + '"',
      "ORDER BY id LIMIT 1000",
      (err, rows) => {
        if (err) return onError(req, res, err.message || "Database error")

        for (var i = 0; i < rows.length; i++) rows[i]["account"] = utils.Encrypt(rows[i].id)
