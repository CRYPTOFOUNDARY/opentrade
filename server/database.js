...
exports.Init = function(callback) {
  ...
  function CreateIndex(indexObject) {
    g_db.run(
      "CREATE INDEX IF NOT EXISTS " + indexObject.name + " ON " + indexObject.table + " (" + indexObject.fields + ")",
      function(err) {
  ...
  function CreateTable(dbTables, nIndex, cbError) {
    ...
    for (var i = 0; i < dbTables[nIndex].cols.length; i++) {
      ...
      if (i != dbTables[nIndex].cols.length - 1) cols += ", "
    }
  ...
  function SelectAll(cols, table, where, other, callback, param) {
    try {
      ...
      if (other.length) query += " " + other

      if (!callback) console.log("WARNING: SelectAll callback undefined!!!")
...
exports.BeginTransaction = function(log, callback, count) {
  ...
  if (g_gotTransaction && counter <= 3) return setTimeout(exports.BeginTransaction, 5000, log, callback, counter + 1)

  if (g_gotTransaction && counter > 3) return callback({message: "Transactions busy " + txLog})
...
exports.RunMemQueries = function(callback) {
  ...
  exports.BeginTransaction(function() {
    g_memQueries.forEach((val, index, array) => {
      ...
      g_db.run(val, error => {
        if (error) {
          //throw 'RunMemQueries unexpected error for query='+val;
