...
function UpdateCoins(message) {
  ...
  $.getJSON("/api/v1/public/getmarkets", ret => {
    ...
    for (var i = 0; i < ret.result.length; i++) {
      const option = $(
        '<option value="' +
          unescape(ret.result[i].MarketCurrencyLong) +
          '">' +
          unescape(ret.result[i].MarketCurrencyLong) +
          "</option>"
      )
...
function onSocketMessage(event) {
  ...
  if (data.request == "coininfo_updated") return UpdateCoins(data.message)
}

function UpdateCoinInfo() {
  const coin = $("#coins-select option:selected").text()
  for (var i = 0; i < g_markets.length; i++) {
    if (g_markets[i].MarketCurrencyLong != coin) continue
