...
$("#id_finduser_balance").submit(e => {
  ...
  $.post("/admin/findbalances", $("#id_finduser_balance").serialize(), function(data) {
    ...
    if (data.result != true) return utils.alert_fail(data.message)
  ...
  for (let i = 0; i < balances.length; i++) {
    const buttonFix = $('<button class="btn btn-default">Fix</button>').on("click", e => {
      ...
      $.getJSON("/fixbalance", {coin: balances[i].coin, userID: balances[i]["userID"]}, ret => {
        $("#loader").hide()
        if (ret.result != true) return utils.alert_fail(ret.message)
    ...
    const tr = $("<tr></tr>")
      ...
      .append(
        $(
          "<td>" +
            (balances[i].deposit * 1 +
              balances[i].buy * 1 +
              balances[i].payouts * 1 -
              balances[i].withdraw * -1 -
              balances[i].sell * 1 -
              balances[i].blocked * 1) +
            "</td>"
        )
      )
      .append(
...
$("#coin-visible").click(function() {
  var info = {}
  try {
    info = JSON.parse($("#coin-info").val())
  } catch (e) {}
  if (this.checked) info["active"] = true
...
$("#coin-minconf").change(e => {
  if (!isInt($("#coin-minconf").val())) return
...
$("#id_finduser").submit(e => {
  ...
  function GetRole(userID, info) {
    let currRole = "User"
    try {
      currRole = JSON.parse(unescape(info)).role
    } catch (e) {}
...
function onSocketMessage(event) {
  ...
  if (!data.request || data.request == "error" || !data.message) return
  ...
  if (data.request == "rpc_responce") {
    if (data.message.result != "success") $("#rpc_responce").text(data.message.data)
    else $("#rpc_responce").text(JSON.stringify(data.message.data))
  }
}
...
function UpdateAdminCoins(data, client_request) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (currentCoin && currentCoin != unescape(data[i].name)) continue
