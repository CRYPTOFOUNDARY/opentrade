...
exports.roundDown = function(number, decimals) {
  try {
    if (!exports.isNumeric(number)) return number

    decimals = decimals || 7

    if (!exports.isNumeric(decimals)) return number
...
exports.UpdateSession = function(userid, token, callback) {
  if (!userid) {
    ...
    if (validTokens[escape(token)]) delete validTokens[escape(token)]

    if (validSessions[escape(token)]) delete validSessions[escape(token)]
...
exports.CheckUserExist = function(user, email) {
  return new Promise((ok, cancel) => {
    IsUserExist(user, exist => {
      if (exist.result == true) return ok({message: "Sorry. This user already registered", info: exist.row})

      IsEmailExist(email, exist => {
        if (exist.result == true) return ok({message: "Sorry. This email already registered", info: exist.row})
  ...
  function IsUserExist(user, callback) {
    if (!user.length) return callback({result: false})

    g_constants.dbTables["users"].selectAll("ROWID AS id, *", "login='" + escape(user) + "'", "", function(
      error,
      rows
    ) {
      if (rows && rows.length) return callback({result: true, row: rows[0]})
  ...
  function IsEmailExist(email, callback) {
    if (!email.length) return callback({result: false})

    g_constants.dbTables["users"].selectAll("ROWID AS id, *", "email='" + escape(email) + "'", "", function(
      error,
      rows
    ) {
      if (rows && rows.length) return callback({result: true, row: rows[0]})
  ...
  function Run(nIndex) {
    ...
    function onEndOne(err, params) {
      if (!cbEndOne) {
        ...
        if (nIndex + 1 < array.length && !err) Run(nIndex + 1)
        else cbEndAll(false) //if all processed then stop and return from 'ForEachSync'
        return
      }
      ...
      cbEndOne(err, params, function(error) {
        ...
        if (nIndex + 1 < array.length) Run(nIndex + 1)
        else cbEndAll(false) //if all processed then stop and return from 'ForEachSync'
      })
    }
  }
...
exports.UpdateRef = function(IP, userID) {
  ...
  g_constants.dbTables["referals"].update('timeReg="' + Date.now() + '", userRegID="' + userID + '"', WHERE, err => {})
}

function InsertRef(req) {
  const uuid = Math.random() * 10000 + "-" + Date.now() + "-" + exports.Encrypt(req.query["ref"])
  const tmpUserID =
    (req.headers["x-forwarded-for"] || req.connection.remoteAddress) + "-" + (Date.now() / (600 * 1000)).toFixed(0)

  if (!exports.isNumeric(req.query["ref"])) return
...
exports.GetSessionStatus = function(req, callback) {
  if (req["query"] && req.query["ref"]) InsertRef(req)

  if (req["session_status"]) return callback(req["session_status"])
  ...
  g_constants.dbTables["sessions"].selectAll("*", 'token="' + token + '"', "", (err, rows) => {
    if (err || !rows || !rows.length) return setTimeout(callback, 10, {active: false, message: errMessage})

    if (Date.now() - rows[0].time > g_constants.SESSION_TIME) {
      g_constants.dbTables["sessions"].delete("time < " + Date.now() + " - " + g_constants.SESSION_TIME)

      if (validSessions[token]) delete validSessions[token]
    ...
    exports.UpdateSession(rows[0].userid, unescape(token), () => {
      g_constants.dbTables["users"].selectAll("ROWID AS id, *", "ROWID='" + rows[0].userid + "'", "", (error, rows) => {
        ...
        validSessions[token]["data"] = {
          ...
          token: token,
          user: rows[0].login,
          password: unescape(rows[0].password),
          email: rows[0].email,
          id: rows[0].id,
          info: rows[0].info
        }
...
exports.GetAllUsersCount = function() {
  if (allUsersCount.count && Date.now() - allUsersCount.time < 1000 * 60) return allUsersCount.count
...
exports.GetValidSessionsCount = function() {
  ...
  if (time - updateKeysTimer > 120000) {
    ...
    for (var key in validTokens) {
      if (!validTokens || !validTokens[key]) continue

      if (time - validTokens[key].time < 120000) tmp[key] = validTokens[key]
    }
    validTokens = tmp
...
exports.render = function(responce, page, info) {
  ...
  render_info["debug"] = false // g_constants.DEBUG_MODE
  render_info["lang"] = lang

  render_info["share"] = g_constants.share
  render_info["app"] = g_constants.share.app
...
exports.validateRecaptcha = function(request) {
  return new Promise((ok, cancel) => {
    ...
    if (g_constants.share.recaptchaEnabled == false) return ok("") //cancel(new Error("Recapcha disabled"))

    if (!request.body || !request.body["g-recaptcha-response"]) return cancel(new Error("Bad Request"))

    exports.postHTTP(
      "https://www.google.com/recaptcha/api/siteverify?secret=" +
        g_constants.recaptcha_priv_key +
        "&response=" +
...
exports.LoadPrivateJS = function(req, res, path) {
  try {
    exports.GetSessionStatus(req, status => {
      if (status.id == 1) return responseFile("./views/pages" + path, res)

      admin_utils.GetUserRole(status.id, info => {
        if (info.role == "Support" && path.indexOf("staff.js") > 0) return responseFile("./views/pages" + path, res)
...
exports.CheckCoin = function(coin, callback) {
  g_constants.dbTables["coins"].selectAll("*", 'name="' + escape(coin) + '"', "", (err, rows) => {
    ...
    try {
      rows[0].info = JSON.parse(exports.Decrypt(rows[0].info))
    } catch (e) {
    ...
    if (rows[0].info.orders == "Disabled")
      return setTimeout(callback, 10, {
        result: false,
        message: 'Coin "' + coin + '" orders is temporarily disabled',
        info: rows[0].info
      })
